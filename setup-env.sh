...
# Автоматическое определение сетевого интерфейса с внешним IP
echo
echo -e "${YELLOW}🔍 Автоматическое определение сетевого интерфейса...${NC}"
WG_DEVICE=$(ip route get 8.8.8.8 | awk 'NR==1 {for(i=1;i<=NF;i++) if($i=="dev") print $(i+1)}')
if [[ -z "$WG_DEVICE" ]]; then
    # Резервный способ определения через маршрут по умолчанию
    WG_DEVICE=$(ip route | grep default | awk '{for(i=1;i<=NF;i++) if($i=="dev") print $(i+1)}' | head -1)
fi
if [[ -z "$WG_DEVICE" ]]; then
    # Если автоматически не определилось, используем eth0 по умолчанию
    WG_DEVICE=eth0
    echo -e "${YELLOW}⚠️  Не удалось автоматически определить сетевой интерфейс, используется eth0${NC}"
else
    echo -e "${GREEN}✅ Обнаружен сетевой интерфейс: $WG_DEVICE${NC}"
fi
...
read -p "Сетевой интерфейс [$WG_DEVICE]: " WG_DEVICE_INPUT
WG_DEVICE=${WG_DEVICE_INPUT:-$WG_DEVICE}
